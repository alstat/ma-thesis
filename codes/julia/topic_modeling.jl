...
# ╔═╡ 8523dec8-ede8-11ee-331f-71daf07f7341
using QuranTree

# ╔═╡ 93b080d3-9e49-46d7-990c-869f0ab6f705
using Yunir

# ╔═╡ 0209fca7-2343-4a19-ad14-8702bc244d28
using TextAnalysis

# ╔═╡ 22bf54de-368b-49c4-96c0-cc689364c858
using PlutoStyles

# ╔═╡ 13eb1e9f-84ed-42c3-93dd-b757cde0c09e
md"# Topic Modeling"

# ╔═╡ adbc0672-ac1b-45e3-b175-653d571f6adf
crps, tnzl = QuranData() |> load;

# ╔═╡ 4f680e98-716c-4afd-b971-7a1bcc38ff35
crpsdata = table(crps)

# ╔═╡ 6e70abb6-b6cf-41e3-8deb-5287af1da459
md"""## Data Processing
THe first data processing will be the removal of all Disconnected Letters, Prepositions, Particles, Conjunctions, Pronouns, and Adverbs. This is done as follows:
"""

# ╔═╡ 0afcbc37-173e-47b4-9b43-67c459416920
function preprocess(s::String)
   ...
   return !disletters && !prepositions && !particles && 
		  !conjunctions && !pronouns && !adverbs
end

# ╔═╡ aebad56c-73f5-4882-9669-5f7c6c28b756
crpstbl = filter(t -> preprocess(t.features), crpsdata[2].data)

# ╔═╡ 8b059627-cf5a-4388-94b2-fa0dc02f832c
md"""
Next, we create a copy of the above data so we have the original state, and use the copy to do further data processing.
"""

# ╔═╡ aed2942e-a0a4-4b02-88b6-27fc072419a7
crpsnew = deepcopy(crpstbl)

# ╔═╡ 1c1f6e47-5e94-488b-8408-0f5f7937fac5
feats_raw = crpsnew[!, :features]

# ╔═╡ 52f0271f-854b-4442-9ccf-4a8418d5f181
feats = parse.(QuranFeatures, feats_raw)

# ╔═╡ 2cf5a806-265c-423e-9bb7-067a34618fa4
md"""
## Lemmatization

Using the above parsed features, we then convert the form of the token into its lemma. This is useful for addressing inflections.
"""

# ╔═╡ ce3ee2fd-7299-4f3d-9a36-ff652c42559c
lemmas = lemma.(feats)

# ╔═╡ 9705cac0-d473-4d24-8962-7b686c9f7ae6
forms1 = crpsnew[!, :form]

# ╔═╡ cfd24e92-e725-4beb-b26b-00f66cdb042e
forms1[.!ismissing.(lemmas)] = lemmas[.!ismissing.(lemmas)]

# ╔═╡ 301d116e-1ec2-47c6-b85b-30b611bce42c
crpsnew[!, :form] = forms1

# ╔═╡ b8e3b37c-f988-42d0-8aba-0d34be0633da
crps_new = CorpusData(crpsnew)

# ╔═╡ 65295e01-edec-4aec-a552-0d82f7124fd8
lem_vrs = verses(crps_new)

# ╔═╡ ff2bd6e1-67b1-4491-a010-7c6e99c01189
arabic.(normalize.(dediac.(lem_vrs)))

# ╔═╡ b1250068-5933-435b-a6a3-3c38b23ad5a2
# vrs = arabic.(replace.(normalize.(dediac.(lem_vrs)), " qAl " => ""))
vrs = replace.(arabic.(normalize.(dediac.(lem_vrs))), 
	"قال" => "", 
	"كان" => ""
)

# ╔═╡ 41ee4a7c-1731-4828-aa9b-e72dc3cb13f8
encode("قال")

# ╔═╡ bbfb287c-e0e6-4f81-9472-be6032e92793
arabic.(vrs)

# ╔═╡ f063d8c9-897d-491a-86d8-bc7f5335b429
crps1 = Corpus(StringDocument.(vrs))

# ╔═╡ c3269901-9f18-4b5a-9aaf-d31ddd32000f
update_lexicon!(crps1)

# ╔═╡ 936b0d91-a32c-49bc-a22d-45450f81a718
update_inverse_index!(crps1)

# ╔═╡ 9a260b1d-8f25-4420-a117-f105e13f6164
m1 = DocumentTermMatrix(crps1)

# ╔═╡ 47e120ac-60c3-40da-9f81-88912759efd6
k = 15          # number of topics

# ╔═╡ 8be56e7a-5265-4499-ad83-95be0665f5d6
iter = 1000    # number of gibbs sampling iterations

# ╔═╡ 2eba128c-3aaf-49b6-add6-06ab7d117030
alpha = 0.1    # hyperparameter

# ╔═╡ 4b5c9a04-eb18-4388-95ce-b0fd979587b8
beta = 0.1     # hyperparameter

# ╔═╡ 5bc2bb19-5578-4ea9-a9a8-2576c9337def
ϕ, θ = lda(m1, k, iter, alpha, beta)

# ╔═╡ 0175db59-2896-49b3-a046-4e926f9f7fe4
ntopics = 10

# ╔═╡ dd9db5cf-f349-45db-b499-34df92ec1e67
cluster_topics = Matrix(undef, ntopics, k);

# ╔═╡ a5c3a725-3d5d-465d-af95-0a7f82d61b7f
for i = 1:k
    topics_idcs = sortperm(ϕ[i, :], rev=true)
    cluster_topics[:, i] = arabic.(m1.terms[topics_idcs][1:ntopics])
end

# ╔═╡ d536d92a-63cc-49ca-82d1-e827bc0c07f6
cluster_topics

# ╔═╡ 103704cc-d3ca-429c-bdce-bc7fa5dff068
vec(cluster_topics)

# ╔═╡ 11726dd2-45e5-480d-a8f9-a11253061cab
m1.terms
...
manifest_format = "2.0"
project_hash = "2819e78a43153e373f837b4bf908723c90beb8d3"
...
git-tree-sha1 = "6a3d14670089d34ee9bd162537cfb4e3890869da"
...
[[deps.PlutoStyles]]
deps = ["Pluto"]
git-tree-sha1 = "9cc856f040557b476b4116c764f339531b178589"
uuid = "16e07271-a41c-4e3e-b73a-b807cf2ea0eb"
version = "0.1.12"
