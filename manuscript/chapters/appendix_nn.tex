...
One limitation of the perceptron is its binary output, which sometimes is poor in discrimination problem, such as in text classification. To address this issue, a new artificial neuron is proposed that returns continuous values between 0 and 1, and so it can be interpreted as probabilities.
...
$$
where $\mathbf{w}_{j}^{(l)}=\left[\begin{array}{cccc}w_{j1}^{(1)}&w_{j2}^{(2)}&\cdots&w_{jd^{(l-1)}}^{(l)}\end{array}\right]^{\top}$. This is computation is propagated up to the output layer. For example, the following is the formula for obtaining $x_{kj}^{(l+1)}$,
$$
\begin{aligned}
x_{kj}^{(l+1)}&=\af\left(z_{kj}^{(l+1)}\right)=\af\left(\left(\mathbf{w}_{j}^{(l+1)}\right)^{\top}\mathbf{x}_k^{(l)} + w_{j0}^{(l)}\right)=\af\left(\sum_{i=1}^{d^{(l)}}w_{ji}^{(l+1)}x_{ki}^{(l)}+w_{j0}^{(l+1)}\right)\\
...
In this section, the error functions of ANN models for regression and classification are derived from probabilistic perspective. For regression setup, the network has at least two nodes (one for the intercept and one for the independent variable) at the input layer and one node at the output layer. The standard activation function is the identity function, i.e. $h(\mathbf{x}_k,\mathbf{w})=z_k=\mathbf{w}^{\top}\mathbf{x}_k$, so there is no hidden layer, see Figure \ref{fig:slrnn}. 
\begin{figure}[t]
\centering\begin{tikzpicture}[
init/.style={
  draw,
...
So for sample data set $\{\mathbf{x}_k,y_k\}$ where $\mathbf{x}_k$ is a $d^{(0)}$-dimensional feature space and $y_k\in\{0,1\}\;\forall k\in\mathbb{N}$, if the $y$s are independent and identically distributed then the likelihood function is given by
$$
...
\label{fig:kloginn}
\end{figure}
Extending the case to at least two separate binary classification requires $d^{(L)}\geq 2$ neurons on the output layer, each of which has a logistic sigmoid activation function. So the target variable is defined as $\mathbf{y}_k=[y_1,y_2,\cdots,y_{d^{(L)}}]^{\top}$, where $y_j\in\{0,1\}\;\forall j\in\mathbb{N}^{(L)}$. Assuming independence on the class labels, the joint conditional distribution of the targets given the input vector is
$$
...
This equation is also known as the \textit{mean squared error}. In order to minimize this function, one has to consider numerical methods such as \textit{gradient descent} or the \textit{stochastic gradient descent}.
