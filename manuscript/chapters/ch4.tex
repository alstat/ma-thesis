...
\begin{figure}[!h]
% \centering\includegraphics[scale = .4]{Neuron}
...
\end{tikzpicture}
\caption[Schematic Representation of the Perceptron Neuron]{\sc Schematic Representation of the Perceptron Neuron.}
...
The following diagram (Figure \ref{fig:logdia}) depicts the flow of the computation in the sigmoid neuron.
\begin{figure}[!h]
\centering\begin{tikzpicture}[
init/.style={
  draw,
...
\draw[decorate,decoration={brace,mirror}] (x1.north west) -- node[left=10pt] {Inputs} (xp.south west);
\end{tikzpicture}
\caption[Diagram of the Logistic Neuron Computation]{\sc Diagram of the Logistic Neuron Computation.}
\label{fig:logdia}
\end{figure}
Also Figure \ref{fig:sigmoid} is the plot of the sigmoid function in a two dimensional space. In the proceeding section, the connections between neurons will be discussed, and how the architecture of these networks is designed.

\begin{figure}[!h]
\centering\begin{tikzpicture}[domain = -10:10, yscale = 5, xscale = 0.5, samples = 200, thick]
\draw[->] (-10.5,0) -- (10.5,0) node[right] {$x$};
\draw[->] (0,-0.1) -- (0,1.1) node[right] {$y$};

\foreach \x/\xtext in {-10/-10, -6/-6, -2/-2, 2/2, 6/6, 10/10}
	\draw[shift={(\x,0)}] (0pt,2/5pt) -- (0pt,-2/5pt) 
	node[below] {$\xtext$};
\foreach \y/\ytext in {.5/0.5, 1/1}
	\draw[shift={(0,\y)}] (2/0.5pt,0pt) -- (-2/0.5pt,0pt) 
	node[left] {$\ytext$};

\draw[color = black, thick] 
	plot[id = p1] function{1/(1+exp(-x))};
\draw[color = black, thick, dotted]
	plot[id = p2] function{1/(1+exp(-.5*x))};
\draw[color = black, thick, dashed] 
	plot[id = p3] function{1/(1+exp(-10*x))};

\draw[color = black, thick] 
	(5.3,.61) -- (6,.61) node[right] {$\af(v)$};
\draw[color = black, thick, dotted] 
	(5.3,.5) -- (6,.5) node[right] {$\af(.5v)$};
\draw[color = black, thick, dashed] 
	(5.3,.39) -- (6,.39) node[right] {$\af(10v)$};
\end{tikzpicture}
\caption[Plot of the Sigmoid Function]{\sc Plot of the Sigmoid Function.}
\label{fig:sigmoid}
\end{figure}
...
\end{tikzpicture}
\caption[Feedforward Neural Network]{\sc Feedforward Neural Network.}
...
\end{tikzpicture}
\caption[Multilayer Feedforward Neural Network]{\sc Multilayer Feedforward Neural Network.}
\label{fig:deepnn}
\end{figure}
...
\end{tikzpicture}
\caption[Neural Net with Standard Neuron Notation]{\sc Neural Net with Standard Neuron Notation.}
\label{fig:nnlab}
\end{figure}
...
\end{tikzpicture}
\caption[Neuron's Schematic Diagram for Logistic Regression]{\sc Neuron's Schematic Diagram for Logistic Regression.}
\label{fig:logrnn}
\end{figure}
...
\end{tikzpicture}
\subcaption{Network Architecture.}
...
\end{tikzpicture}
\subcaption{Neuron's Schematic Diagram.}
\end{subfigure}
...
midarrow/.style={decoration={markings, mark=at position 0.5 with {\arrow{stealth}}},postaction={decorate}},
start chain=4,node distance=20mm, thick
]
\begin{scope}[start chain=1]
\node[on chain=1] at (0,5cm) 
  (x1) {$x_{k1}^{(0)}$};
\end{scope}

\begin{scope}[start chain=2]
\node[on chain=2] at (0,3cm) 
  (x2) {$x_{k2}^{(0)}$};
\end{scope}
...
\node[on chain=4]
  (xi1) {$x_{ki}^{(0)}$};
\node[on chain=4,init, join=by o-] (sigma1) 
  {$\displaystyle\Sigma$};
\node[on chain=4] 
  (xiL) {$x_{kj}^{(L)}$};
\node[on chain=4] 
...
\node[on chain=4] at (3.6,5cm) 
  (x11) {$x_{k1}^{(0)}$};
\node[on chain=4] at (3.6,3cm) 
  (x21) {$x_{k2}^{(0)}$};
\node[on chain=4] at (3.7,1cm) 
  {$\vdots$};
\node[on chain=4] at (3.7,-1cm) 
  {$\vdots$};
\node[on chain=4] at (3.6,-3cm) 
...
\begin{scope}[start chain=5,node distance=20mm]
\node[on chain=5] at (0,-1cm) 
  {$\vdots$};
\end{scope}

\begin{scope}[start chain=6]
\node[on chain=6] at (0,-3cm) 
  (xp) {$x_{kd^{(0)}}^{(0)}$};
\end{scope}

\node[label=below:\parbox{2cm}{\centering Intercept \\ $w_0=+1$}] at (sigma|-xp) (b) {};
\node[label=below:\parbox{2cm}{\centering Intercept \\ $w_0=+1$}] at (sigma1|-xp1) (b1) {};

\draw[midarrow,o-] (x1) -- node[above right] {$w_{j1}^{(1)}$} (sigma);
\draw[midarrow,o-] (x2) -- (sigma);
\draw[midarrow,o-] (xi) -- node[above] {$w_{j1}^{(1)}$} (sigma);
\draw[midarrow,o-] (xp) -- node[above left] {$w_{j1}^{(1)}$} (sigma);
\draw[o-latex] (b) -- (sigma);
\draw[midarrow] (sigma) -- node[above] {$\af(\cdot)$} (xi1);
\draw[midarrow] (sigma1) -- node[above] {$\af(\cdot)$} (xiL);


\draw[midarrow,o-] (x11) -- node[above right] {$w_{j1}^{(1)}$} (sigma1);
\draw[midarrow,o-] (x21) -- (sigma1);
\draw[midarrow,o-] (xi1) -- node[above] {$w_{j1}^{(1)}$} (sigma1);
\draw[midarrow,o-] (xp1) -- node[above left] {$w_{j1}^{(1)}$} (sigma1);
\draw[o-latex] (b1) -- (sigma1);

\draw[midarrow,o-o] (xiL) -- node[above] {$-1$} (y);
\draw[-latex] (y) -- (e);
